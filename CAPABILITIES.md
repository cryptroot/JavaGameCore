...
| CPU-bound parallel range work | `core.concurrent.WorkerPool` | dedicated `ForkJoinPool` wrapper; `parallelFor`/`mapChunks` split `[from,to)` into chunks, run them, block until all finish (the "gate"), then return — for read-only/embarrassingly-parallel work only, one instance per `GameContext` (`workerPool()`) |
